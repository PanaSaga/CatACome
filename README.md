...
흘러내린다) · 모래암반 낙하와 매몰 · 다이너마이트 3연쇄 · 가시발판 · 굴러떨어지는
바위(경도 3 지대에만, 밑을 파내면 떨어져서 굴러간다) ·
...
- **곡괭이 범위·공격력·속도**는 서로 완전히 분리된 세 트랙이다. 범위(1~3레벨)는
  오직 파괴 면적(1x1~3x3)만 정하고 경도가 세다고 줄지 않는다. 필요 타수는 오직
  공격력(1~3레벨, 기본 1)과 그 경도로만 정해진다(공격력1·경도3 지형 → 3타). 속도는
  기존처럼 1~8레벨. "곡괭이 등급"(은행 수수료·의료소 비용)은 속도 레벨만 따른다.
